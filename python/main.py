...
def main():
    ...
    database.write_df_to_sql(config, movies_data.make_movies_df(df), 'movies_metadata')
    database.write_df_to_sql(config, movies_data.make_studios_df(df), 'production_companies')
    database.write_df_to_sql(config, movies_data.make_genres_df(df), 'movie_genres')
